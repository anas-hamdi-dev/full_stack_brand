...
export function updateProduct(
  ...
  const productIndex = staticProducts.findIndex(p => p.id === productId);
  if (productIndex === -1) {
    throw new Error(`Product with id ${productId} not found`);
  }

  const existingProduct = staticProducts[productIndex];
  const brand = staticBrands.find(b => b.id === (updates.brand_id || existingProduct.brand_id));
  ...
  staticProducts[productIndex] = updatedProduct;
...
export function deleteProduct(productId: string): void {
  ...
  staticProducts.splice(productIndex, 1);
}
