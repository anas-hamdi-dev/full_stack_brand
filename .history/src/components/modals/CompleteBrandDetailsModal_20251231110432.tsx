...
export default function CompleteBrandDetailsModal({
  ...
  const handleOpenChange = (newOpen: boolean) => {
    // Only allow closing if user explicitly clicks "Plus tard" or after successful submission
    // Prevent closing by clicking outside or pressing ESC
    if (!newOpen && !isLoading) {
      // Only close if it's a programmatic close (not user interaction)
      // We'll handle this through the button click handlers
      return;
    }
        ...
        <div className="space-y-6 mt-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <Button
                type="submit"
                className="flex-1 bg-muted text-foreground hover:bg-muted/80 h-12"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
