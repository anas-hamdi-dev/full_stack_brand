...
const _staticProducts: Product[] = [...staticProducts];

// Function to add a new product
export function addProduct(product: Omit<Product, "id" | "created_at" | "brands">): Product {
  const brand = staticBrands.find(b => b.id === product.brand_id);
  ...
  _staticProducts.push(newProduct);
  return newProduct;
}
...
export function updateProduct(
  productId: string,
  updates: Partial<Omit<Product, "id" | "created_at" | "brands">>
): Product {
  const productIndex = _staticProducts.findIndex(p => p.id === productId);
  if (productIndex === -1) {
    throw new Error(`Product with id ${productId} not found`);
  }

  const existingProduct = _staticProducts[productIndex];
  const brand = updates.brand_id 
    ? staticBrands.find(b => b.id === updates.brand_id)
    : staticBrands.find(b => b.id === existingProduct.brand_id);
  ...
  const updatedProduct: Product = {
    ...
    brands: brand ? {
      ...
      logo_url: brand.logo_url,
      website: brand.website,
    } : existingProduct.brands,
  };

  _staticProducts[productIndex] = updatedProduct;
...
export function deleteProduct(productId: string): void {
  ...
  _staticProducts.splice(productIndex, 1);
}

// Export getter for static products (mutable)
